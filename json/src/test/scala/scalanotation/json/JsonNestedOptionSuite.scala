...
class JsonNestedOptionSuite extends munit.FunSuite:
  ...
  private def roundTrip[T: ReadWriter](value: T, expected: String): Unit =
    val rendered = Json.write(value)
    assertEquals(rendered, expected)
    assertEquals(Json.readAs[T](rendered), Result.Ok(value))
    assertEquals(Json.readAs[T](Json.writeBytes(value)), Result.Ok(value))
